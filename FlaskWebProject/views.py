...
def delete_post(id):
    post = Post.query.get(int(id))
    post.delete_post()
    flash('Deleted post with ID {0}'.format(post.id))
    return redirect(url_for('home'))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            ...
            app.logger.warning('Invalid login attempt for user "{username}".'.format(username=form.username.data))
            return redirect(url_for('login'))
        login_user(user, remember=form.remember_me.data)
        next_page = request.args.get('next')
        if not next_page or url_parse(next_page).netloc != '':
            next_page = url_for('home')
        app.logger.info('User "{username}" logged in successfully.'.format(username=form.username.data))
...
def authorized():
    ...
    if request.args.get('code'):
        ...
        login_user(user)
        _save_cache(cache)
        app.logger.info('"{email}" logged in successfully as user "{username}".'.format(username=user.username,email=result.get('id_token_claims')['preferred_username']))
    return redirect(url_for('home'))

@app.route('/logout')
def logout():
    if current_user.is_authenticated:
        app.logger.info('User "{username}" logged out successfully.'.format(username=current_user.username))
    logout_user()
